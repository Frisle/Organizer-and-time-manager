...
def main():
    ...
    def take_notes():
        ...
        if "-copy" in user_prompt:
            ...
            print(write_create.read_json_display_tags("tags", json_data_list_file))
            take_notes()
            ...
            write_create.read_json_date_func(transform_string, json_data_file)
            take_notes()
            ...
            print("Input \"-New\" and write a new note. Write your tag after it if needed\n"
                  "Input \"-cls\" to clear the console\n"
                  "Input \"-sh\" to search the notes by key word\n"
                  "Input \"-tag\" to search the notes by tag word"
                  "Input \"-copy\" to use clipboard. Write your tag after it if needed\n"
...
# test_case --file-version=0.11.0.11 --product-name=Organizer --enable-console --mingw64 --standalone --onefile --windows-icon-from-ico=organizer_ico.ico --output-dir="C:\Users\wda61\PycharmProjects\Builds\Organizer" --remove-output
# --file-version=0.11.0.11 --product-name=Organizer --enable-console --mingw64 --standalone --onefile --windows-icon-from-ico=organizer_ico.ico --output-dir="C:\Distributed_apps" --remove-output
